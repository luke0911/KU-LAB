...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    // 영점 기준 시간 (센서 샘플링 시작 시각)
    private var startTime: Long = 0

    // 계단 버튼 상태 및 현재 층 (초기값 1층)
    private var stairsPressed: Boolean = false

    // 로그 데이터 (각 샘플의 문자열 기록)
    private val logData = ArrayList<String>()

    // 버튼 참조 (XML에 btnAscend, btnDescend가 정의되어 있어야 함)
        ...
        btnAscend.setOnTouchListener { v, event ->
            when (event.action) {
                ...
                MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                    stairsPressed = false
                    // 버튼 원래 배경색으로 복원 (예: #B6DAF3)
                    v.setBackgroundColor(Color.parseColor("#B6DAF3"))
                    // 상승: 만약 현재 층이 1이면 -1로 변경, 그 외에는 +1
        ...
        override fun onSensorChanged(event: SensorEvent?) {
            event?.let {
                ...
                if (heroPDR.isStep(it.values.clone(), accMatrix)) {
                    ...
                    step_length = pdrResult.stepLength.toFloat() - 0.035f
                    handleStepDetection()
                }
            }
        }
    ...
    private fun saveDataToFile() {
        ...
        try {
            DataOutputStream(openFileOutput(fileName, MODE_APPEND)).use { dos ->
                // 첫 번째 행에 컬럼명 기록
                val header = "시간\t가속도x\t가속도y\t가속도z\t기압\t자이로\t걸음유무\t계단유무\t층\n"
            ...
            when (it.sensor.type) {
                ...
                Sensor.TYPE_LINEAR_ACCELERATION -> {
                    ...
                    if (accMatrix != null && magMatrix != null) {
                        val R = FloatArray(9)
                        if (SensorManager.getRotationMatrix(R, null, accMatrix, magMatrix)) {
                            val worldAcc = FloatArray(3)
                            worldAcc[0] = R[0] * linAcc[0] + R[1] * linAcc[1] + R[2] * linAcc[2]
                            worldAcc[1] = R[3] * linAcc[0] + R[4] * linAcc[1] + R[5] * linAcc[2]
                            worldAcc[2] = R[6] * linAcc[0] + R[7] * linAcc[1] + R[8] * linAcc[2]
                            globalX = worldAcc[0]
                            globalY = worldAcc[1]
                            globalZ = worldAcc[2]
                        }
                    ...
                    // 로그 기록 시, stepDetectionListener에서 업데이트된 stepFlagShared 값을 사용
                    val currentStepFlag = stepFlagShared
                    // 로그 후에는 초기화 (또는 다른 방식으로 관리)
                    stepFlagShared = 0

                    val elapsedTime = (System.currentTimeMillis() - startTime) / 1000.0
                    val stairsFlag = if (stairsPressed) 1 else 0
                    ...
                    val logLine = String.format(
                        ...
                        linAcc[1].toDouble(),
                        linAcc[2].toDouble(),
                        lastPressure.toDouble(),
                        updatedAngle.toDouble(),
                        currentStepFlag,
                        stairsFlag,
                        cur_floor_int
                    )
...
//        // Save step data
//        val stepData = arrayOf(
//            nextPosition[0].toString(),
//            nextPosition[1].toString(),
//            updatedAngle.toString(),
//            (step_length - 0.035f).toString(),
//            compassDirection.toString()
//        )
//        history_of_data.add(stepData)
    }
